...
| Control Loop CLI vs internals | Stable: `mpc-control-loop` CLI flags (`--apply`, `--dry-run`, `--log-csv`). Unstable: internal Python module layout outside documented CLIs (e.g. direct imports from `analysis/` submodules). | [`analysis/README.md#cli-commands`](../analysis/README.md#cli-commands) |
